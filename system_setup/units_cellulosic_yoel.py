# -*- coding: utf-8 -*-
#Copied from Yoel oilcane biorefinery
"""
.. contents:: :local:
...
class PretreatmentReactorSystem(bst.units.design_tools.PressureVessel, Unit):
    ...
    def __init__(self, ID='', ins=None, outs=(), T=130+273.15, thermo=None, 
                 tau=5/60, V_wf=0.8, length_to_diameter=2,
...
@cost('Annual flow rate', 'Membranes and maintenance', S=1., units='m3/yr',
      ...
      annual=True)
class Nanofilter(bst.Unit): #modified to MWCO200 membrane, seperates sugars and smaller chemicals (HMF,furfural, acetic acid, ect.)
    _N_ins = 1
    _N_outs = 2
    _units = {'Annual flow': 'kg'}
    #area, design flow rate, pressure
    #use storage tank before NF to maintain flow rate for batch filtration
    def __init__(self, ID = '', ins = None, outs = (), thermo = None, *, volume_reduction,sugar_retention,hmf_retention,furfural_retention,aceticacid_retention):
        bst.Unit.__init__(self, ID, ins, outs, thermo)
        self.volume_reduction = volume_reduction
        self.sugar_retention = sugar_retention
        self.hmf_retention = hmf_retention
        self.furfural_retention = furfural_retention
        self.aceticacid_retention = aceticacid_retention

    # volume_reduction = 0.80
    # sugar_retention = 0.99
    # value_added_retention = 0.4
    # aceticacid_retention = 0.8

    sugars_IDs = ('Glucose', 'Xylose', 'Arabinose', 'Galactose', 'Mannose',
              'GlucoseOligomer', 'GalactoseOligomer', 'MannoseOligomer',
              'XyloseOligomer', 'ArabinoseOligomer')
    hmf_IDs = ('HMF',)
    furfural_IDs = ('Furfural',)
    aceticacid_IDs = ('AceticAcid',)
    def _run(self):
        feed, = self.ins
        ...
        hmf_retention = self.hmf_retention
        furfural_retention = self.furfural_retention
        aceticacid_retention = self.aceticacid_retention
        ...
        # not_permeate_unique = (*self.sugars_IDs, 'Water',*self.hmf_IDs, *self.aceticacid_IDs, *self.furfural_IDs)
        ...
        hmf_comps = feed.imass[self.hmf_IDs]
        furfural_comps = feed.imass[self.furfural_IDs]
        acetic_comps = feed.imass[self.aceticacid_IDs]
        # NaOH = feed.imass['NaOH']
        # lignin = lignin_comps.sum()
        sugar = sugar_comps.sum()
        hmf = hmf_comps.sum()
        furfural = furfural_comps.sum()
        aceticacid = acetic_comps.sum()
        # lignin_def = lignin_comps / lignin
        sugar_def = sugar_comps / sugar
        hmf_def = hmf_comps / hmf
        furfural_def = furfural_comps / furfural
        acetic_def = acetic_comps / aceticacid
        # NaOH = feed.imass['NaOH']
        feed_mass = np.array([hmf, furfural, sugar, aceticacid])
        z_mass = feed_mass / F_mass
        K = np.array([hmf_retention,furfural_retention, sugar_retention, aceticacid_retention])
        # phi_guess = 0.5
        Fa = permeate.F_mass
        Fb = retentate.F_mass
        assert abs((Fa + Fb + hmf + furfural + sugar + aceticacid) - F_mass) < 1e-6
        ...
        permeate.imass[self.hmf_IDs] = permeate_mass[0] * hmf_def
        permeate.imass[self.furfural_IDs] = permeate_mass[1] * furfural_def
        permeate.imass[self.sugars_IDs] = permeate_mass[2] * sugar_def
        permeate.imass[self.aceticacid_IDs] = permeate_mass[3]
        retentate.mol = feed.mol - permeate.mol
        assert (permeate.mol >= 0.).all()
        retentate.T = permeate.T = feed.T
    def _design(self):
        Design = self.design_results
        Design['Annual flow'] = self.feed.F_mass*8000

    def _cost(self):
        flow = self.design_results['Annual flow']
        purchase_cost = bst.settings.CEPCI*3.08*flow**0.55 #temporary wrong cost number for testing, YJ
        self.baseline_purchase_costs['Nanofilter'] = purchase_cost
        self.F_D['Nanofilter'] = self. F_P['Nanofilter'] = self.F_M['Nanofilter'] = 1.0
        self.F_BM['Nanofilter'] = 2.45 #tempoerary wrong number for testing
