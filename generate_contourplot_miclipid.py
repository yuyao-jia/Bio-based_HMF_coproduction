...
results_path='contourplots/microbial_lipid/test4'

# retention contourplots
x_data = np.linspace(0.4,0.7,steps) #microbial lipid yield from glucose
y_data = np.linspace(8.8,28,steps) #microbial lipid titer
...
def metrics_at_x_and_y_retention(x,y):
    F.unit.R401.cofermentation[0].product_yield(
        "TAG", basis="wt", product_yield=0.33 * x
    )
...
results = pd.DataFrame(columns=['Microbial lipid yield from glucose [%]','Microbial lipid titer [g/L]',]
# results = pd.DataFrame(columns=['Microbial lipid yield from glucose [%]',]
                               +metric_names)
...
#     xi = i
#     ri = metrics_at_x_and_y_retention(xi, 0)
#     # di = [xi, yi] + ri
#     di = [xi] + ri
#     results.loc[i] = di


results.to_excel(f"{results_path}/{date_formatted}_miclipid_{steps}_results.xlsx")
baseline_metrics.to_excel(f"{results_path}/{date_formatted}_baseline_metrics_for_verification.xlsx")


# data = pd.read_excel(f"{results_path}/06262024_miclipid_30_results.xlsx",usecols='B:CT')
# metric_names = [i[1] for i in baseline_metrics.index]
# min1 = pd.DataFrame(columns=['Microbial lipid yield from glucose [%]']+metric_names)
