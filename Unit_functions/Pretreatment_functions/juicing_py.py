...
from Unit_functions.Pretreatment_functions.bagasse_yj import (
...
def create_feedstock_handling_system(ins, outs):
    sugarcane, = ins
    shreaded_cane, = outs
    #No storage tank for oilcane: assume immediate processing upone receiving
    U101 = bst.ConveyingBelt('U101', sugarcane)
    U102 = bst.MagneticSeparator('U102', U101-0, outs=['', 'separated_metal'])
...
def create_juicing_system_without_treatment(ins, outs, pellet_bagasse=None,
    ...
    U201 = bst.CrushingMill('U201',
                              ...
                              split=dict(Ash=0.92,
                                         ...
                                         Sucrose=0.04,
                                         Solids=1,
                                         Vegetative_lipid = 0.95),
                              moisture_content=0.5)
    U201.isplit['Vegetative_lipid'] = 0.95  # Crushing mill, 95% of lipids stay in bagasse
    ...
    S201 = bst.VibratingScreen('S201', U201-1, ('', 0-M201),
                                 split=dict(Ash=0.35,
                                            ...
                                            Sucrose=0.88,
                                            Water=0.88))
    S201.isplit['Lipid'] = 1.0  # Fiber screener #1, all lipids in juice (5% of total lipid) stays in juice
    # Store juice before treatment
    T202 = bst.StorageTank('T202', S201-0, untreated_juice,
                             tau=4, vessel_material='Carbon steel')

    # assuming all lipids remain in bagasse for easy calculation of overall lipid distribution in solids/liquids
...
def create_juicing_system_up_to_clarification(ins, outs, pellet_bagasse=None,
    ...
    C202 = bst.RVF('C202', (C201-1, rvf_wash_water), outs = (filter_cake, ''),
                     ...
                     split=dict(Ash=0.85,
                                ...
                                Lignin=0.85,
                                Sucrose=0.01))
    P203 = bst.Pump('P203', C202-1, 1-M202) #recycle liquid separated from filter cake by C202 RVF
    ...
    def correct_wash_water():
        P202._run()#_run() runs the unit without specification
...
@SystemFactory(
    ID='juicing_sys',
    ins=[s.sugarcane, s.H3PO4, s.lime, s.polymer],
    outs=[s.screened_juice, s.bagasse, s.fiber_fines,s.filter_cake]
)          
def create_juicing_system(ins, outs, pellet_bagasse=None, dry_bagasse=None):
    screened_juice, bagasse, fiber_fines,filter_cake = outs
    sys = create_juicing_system_up_to_clarification(
        None, ins, ['', bagasse,filter_cake],
        mockup=True,
        pellet_bagasse=pellet_bagasse,
        dry_bagasse=dry_bagasse,
    )
