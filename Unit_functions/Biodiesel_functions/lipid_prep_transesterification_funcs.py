...
.. autofunction:: biorefineries.biodiesel.system_setup.create_lipid_wash_system
.. autofunction:: biorefineries.biodiesel.system_setup.create_lipid_pretreatment_system
.. autofunction:: biorefineries.biodiesel.system_setup.create_transesterification_and_biodiesel_separation_system
...
from biorefineries.biodiesel import units
...
@SystemFactory(
    ...
    ins=[s.crude_vegetable_oil, s.pure_glycerine],
    outs=[s.degummed_oil]
)
def create_lipid_pretreatment_system(ins, outs):
    crude_vegetable_oil, pure_glycerine = ins
    degummed_oil = outs
    # #Water is vacuumed out in lipid extraction systems
    # # Vacume out water
    # #Water negatively affect glycerolysis
    # # F3 = bst.SplitFlash('F3', T=357.15, P=101325,
    ...
    # H6 = bst.HXutility('H6', crude_vegetable_oil, T=320, rigorous=False)
    N2 = bst.Stream('N2', phase='g')
    # acetone_recycle = bst.Stream()
    # #Skimming tank to separate polar lipids by crystallization in acetone
    ...
    P3 = bst.Pump('P3', ins[0], P=101325)
    # F1 = bst.Flash('F1', P3-0, V=1., P=101325)
    H1 = bst.HXutility('H1', P3-0, T=320, V=0, rigorous=False)
    # P4 = bst.Pump('P4', H1-0, P=101325)
    ...
    # M1 = bst.Mixer('M1', [P5-0])
    # bst.mark_disjunction(acetone_recycle)
    # P4 = bst.Pump('P4', P5-0, P=101325)
    hx_stream = bst.Stream()
    H3 = bst.HXprocess('H3', [H1-0, hx_stream])
    glycerol_recycle = bst.Stream()
    M2 = bst.Mixer('M2', [P2-0, glycerol_recycle])
    #oleochemicals.systems_baseline.hosun: degumming units process
    R1 = units.GlycerolysisReactor('R1', [H3-0, M2-0, N2],outs=['vent_form_glycerolysis',''])
...
@SystemFactory(
    ID="lipid_pretreatment_sys_with_degumming",
    ins=[s.crude_vegetable_oil, s.acetone, s.pure_glycerine],
    outs=[s.degummed_oil, s.polar_lipids]
)
def create_lipid_pretreatment_system_with_degumming(ins, outs):
    ...
    #                     split=dict(Lipid=0.00001,
    #                                Water=1.0), )
    P10 = bst.Pump('P10', crude_vegetable_oil, P=101325)
    # H5 = bst.HXutility('H5', ins=F3 - 0, T=320, V=0)
    # P9 = bst.Pump('P9', H5 - 0, wastewater, P=101325)
    T1 = bst.StorageTank('T1', acetone, tau=7 * 24)
    P1 = bst.Pump('P1', T1 - 0, P=101325.)
