...
def create_facilities(
    ...
    with bst.MockSystem() as sys:
        ...
        if feedstock is not None:
            ...
            if ADP:
                plant_air = bst.Stream('plant_air', N2=79,O2=21, units='kg/hr')
                #only assumed to be N2 since pure N2 is needed in glyceryolysis, but is actually air and free except for air to R601
                ...
                def adjust_plant_air():
                    plant_air.imass['N2'] = feedstock.F_mass * ADP.plant_air_over_feedstock * 0.79
                    plant_air.imass['O2'] = feedstock.F_mass * ADP.plant_air_over_feedstock * 0.21
            if FWT:
...
def create_coheat_and_power_system(
    ...
    BT = bst.BoilerTurbogenerator(
        ...
        outs=outs,
        satisfy_system_electricity_demand=False, #buy electricity from the grid if need additional electricity
        **kwargs,
    )
