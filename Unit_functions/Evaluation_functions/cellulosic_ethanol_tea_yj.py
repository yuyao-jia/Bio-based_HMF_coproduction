...
def create_cellulosic_ethanol_tea(sys, OSBL_units=None, cls=None):
    ...
    tea = cls(
        system=sys, 
        IRR=0.10, 
        duration=(2023, 2053),#30 years of construction duration
        depreciation='MACRS7', 
        income_tax=0.35,
...
def capex_table(teas, names=None):
    ...
    tea, *_ = teas
    ISBL_installed_equipment_costs = np.array([i._ISBL_DPI_cached / 1e6 for i in teas])
...
def foc_table(teas, names=None):
    ...
    ISBL = np.array([(i._ISBL_DPI_cached) / 1e6 for i in teas])
