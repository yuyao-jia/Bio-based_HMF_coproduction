...
def set_lipid_fraction(lipid_fraction, stream=None,
    ...
    lipid_IDs = tuple([i for i in ('PL_veg', 'FFA_veg', 'MAG_veg', 'DAG_veg', 'TAG_veg') if i in thermo.chemicals])
    carbs = Stream(None, thermo=thermo)
    fiber = Stream(None, thermo=thermo)
    lipid = Stream(None, TAG_veg=1.0 - PL_fraction - FFA_fraction, units='kg/hr', thermo=thermo)
    if PL_fraction: lipid.imass['PL_veg'] = PL_fraction
    if FFA_fraction: lipid.imass['FFA_veg'] = FFA_fraction
...
class CaneCompositionSpecification:
    ...
    def __init__(self, feedstock, oil=None, moisture=None, fiber=None, FFA_veg=None, PL_veg=None):
        ...
        self.FFA = composition['FFA_veg'] if FFA_veg is None else FFA_veg
        self.PL = composition['PL_veg'] if PL_veg is None else PL_veg
