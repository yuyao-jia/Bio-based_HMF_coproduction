...
results_path='contourplots/lipidcombined/test1'

x_data = np.linspace(0.4,1.0,steps) #microbial lipid yield from glucose
# y_data = np.linspace(8.8,30,steps) #microbial lipid titer
...
results = pd.DataFrame(columns=['Microbial lipid yield from glucose [%]','Vegetative lipid recovery after pretreatment & saccharification [%]',]
                               +metric_names)
...
results.to_excel(f"{results_path}/{date_formatted}_combinedlipid_{steps}_results.xlsx")
